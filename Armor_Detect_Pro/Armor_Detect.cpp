        ...
        for (auto male_light : lights)
        {
            ...
            for (auto female_light : lights)
            {
                if (female_light != male_light)
                {
                    ...
                    if (area_ratio_diff < MATCH_COND.MAX_AREA_RATIO_DIFF && perimeter_ratio_diff < MATCH_COND.MAX_PERIMRTER_RATIO_DIFF)
                    {
                        if (width_ratio_diff < MATCH_COND.MAX_WIDTH_RATIO_DIFF && height_ratio_diff < MATCH_COND.MAX_HEIGHT_RATIO_DIFF)
                        {
                            if (angle_ratio_diff < MATCH_COND.MAX_ANGLE_DIFF && center_dis_diff < 1.5 * male_light.get_height())
                            {
